...
export class MercedesApi {
  ...
  async getVehicles(): Promise<Vehicle[]> {
    ...
    const result = vehicles.map((v) => ({ vin: String(v.vin ?? v.fin ?? v.finorvin) }));
    this.log.debug('MercedesApi: getVehicles found %d vehicle(s): %j', result.length, result.map((v) => v.vin));
    return result;
  }
  ...
  async getVehicleStatus(vin: string): Promise<CarStatus> {
    ...
    this.log.debug('MercedesApi: getVehicleStatus(%s) received %d bytes', vin, buffer.length);
